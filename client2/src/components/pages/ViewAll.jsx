...
export default class ViewAll extends Component {
  ...
  render() {
    ...
    if (this.state.isLoading === true) {
      ...
      let filteredResults = Array.from(this.state.studentData).filter(
        (student) => regexp.test(Object.values(student))
      );
      profileEntry = filteredResults.map((student) => (
        <ProfileEntry studentData={student} />
      ));
    } else {
      profileEntry = Array.from(this.state.studentData).map((student) => (
        <ProfileEntry studentData={student} />
      ));
    }
    ...
    return (
      <Section className="profileList_container" isCentered>
        ...
        <Container>
          <Title className="profileList_title" isSize={3}>
            View All Graduate Profiles
          </Title>
          <Columns isCentered>
            <Column isSize="1/5">
            ...
            <Column isSize="3/4">{profileEntry}</Column>
          </Columns>
        </Container>
      </Section>
    );
